...
import { Star, Scissors, Heart, Eraser, Gift, Check, RefreshCw, Lock } from 'lucide-react';
...
const PRICING_OPTIONS: PricingOption[] = [
  ...
  { 
    ...
    pricePerUnit: 26.00, 
    totalPrice: 52.00, 
    originalPrice: 144.00, // Matching the screenshot's specific math
    isPopular: true, 
    count: 2 
  },
  { 
...
export const ProductSection: React.FC = () => {
  ...
  const selectedOption = PRICING_OPTIONS.find(o => o.id === selectedOptionId) || PRICING_OPTIONS[1];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="lg:w-1/2 flex flex-col gap-6">
            ...
            {/* Header Info */}
            <div>
              <h2 className="font-display text-4xl lg:text-5xl text-slate-900 mb-4 leading-tight">Hydrocolloid Treatment</h2>
              <p className="text-slate-600 text-lg leading-relaxed">
                Get clearer, smoother, and acne-free skin overnight without breaking the bank. Are you ready to look your best?
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <div key={idx} className="px-3 py-1.5 rounded-full border border-gray-200 bg-white flex items-center gap-1.5 text-xs font-semibold text-slate-600 shadow-sm">
            ...
            <div className="grid grid-cols-3 gap-3">
              {PRICING_OPTIONS.map((option) => (
                ...
                >
                  ...
                  <div className={`font-bold text-lg mb-1 ${selectedOptionId === option.id ? 'text-primary' : 'text-slate-900'}`}>
                    {option.label}
                  </div>
                  <div className={`text-sm ${selectedOptionId === option.id ? 'text-slate-700 font-medium' : 'text-slate-500'}`}>
            ...
            <div className="mt-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 rounded-lg border border-primary bg-white shadow-sm relative overflow-hidden group">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-14 h-14 bg-orange-50 rounded-md flex-shrink-0 overflow-hidden border border-gray-100">
                      <img src="https://lh3.googleusercontent.com/aida-public/AB6AXuCqK068dwPAIvJyJ6Ce5pDQODLZZ8xXiwfis9drBWr7Q4o0779bewhrF69tSVj17BT2eGZJdsqK8hqI257eXDz8n4WQza6mCdAhwbXjsVEaTUK0ZCy7-HoZFK9rm4If8u56f7b6PnHQcWbR-WJJKRj4SjxEwWknLlWed9RU9IhyneIn-VgzhP86yafqs-BDdD2xFZqaFjxXwcnWuvsnYynTmaVKvfxs0rpo937BkT9D9c0vm3xtd_IiSJGYTLdvJJm9bC40sM816A" alt="Kit" className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
                    </div>
                    <div>
                      <div className="font-bold text-slate-900 text-sm">Acne Remover Kit</div>
                      <div className="text-xs text-slate-500 mt-0.5">Complete acne removal kit to enhance your skincare routine.</div>
                    </div>
                  </div>
                  <div className="text-right pl-2">
                    <div className="text-xs line-through text-slate-400 mb-0.5">$24</div>
                    <div className="text-xs font-bold text-primary bg-primary/10 px-2 py-0.5 rounded">FREE</div>
                  </div>
                </div>
                ...
                `}>
                   {!isSerumLocked && <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary"></div>}
                  <div className="flex items-center gap-3">
                    <div className={`w-14 h-14 bg-pink-50 rounded-md flex-shrink-0 overflow-hidden border border-gray-100 relative ${isSerumLocked ? 'grayscale' : ''}`}>
                       {isSerumLocked && (
                         <div className="absolute inset-0 flex items-center justify-center bg-black/10 z-10">
                           <Lock size={16} className="text-white" />
                         </div>
                       )}
                      <img src="https://lh3.googleusercontent.com/aida-public/AB6AXuD2t6aT_lETDjorA-9uREJCLkD6OSKjLI-gYtr_EEYC04UWIQJxeo0lrIWDus-TBtMG07xIkjBYND-ov6lF2iL-bSnPJH_3Av26GCL4aE94Dn3awpkxvFM_jMFF-ZNJpjS2W89uoY7QGLy5Kr8k5JYd0qc-6I-cDt0LgQxC1Z4APD62bYe_jqEW6B6fi62_ubB7jT7FF635bk0dL8-BCy3RKIAJzFygfOfJdGegVz6ATL7DcoA9pFGntkyS-JJO8QLN1CZfdoXFHA" alt="Serum" className="w-full h-full object-cover" />
                    </div>
                    <div>
                      <div className="font-bold text-slate-900 text-sm">Niacinamide Serum</div>
                      <div className="text-xs text-slate-500 mt-0.5">Premium niacinamide serum for brighter, clearer skin.</div>
                      {isSerumLocked && <div className="text-[10px] text-accent-red font-medium mt-1">Select 3 Rolls to unlock</div>}
                    </div>
                  </div>
                  <div className="text-right pl-2">
                    <div className="text-xs line-through text-slate-400 mb-0.5">$49</div>
                    <div className={`text-xs font-bold px-2 py-0.5 rounded ${isSerumLocked ? 'text-slate-500 bg-slate-200' : 'text-primary bg-primary/10'}`}>
            ...
            <div className="mt-4 pt-6 border-t border-gray-200">
              <div className="flex items-baseline gap-3 mb-6">
                <span className="text-4xl font-bold font-display text-slate-900">${selectedOption.totalPrice}</span>
                <span className="text-xl text-gray-400 line-through decoration-2">${selectedOption.originalPrice}</span>
            ...
            <div className="mt-8 space-y-2 border-t border-gray-200 pt-6">
              <Accordion title="How Does It Work?">
                Our hydrocolloid patches absorb impurities and protect your skin, creating the perfect environment for faster healing. The patch draws out pus and fluids while preventing you from picking at the blemish.
              </Accordion>
              <Accordion title="What If It Doesn't Work?">
                We offer a 90-day money-back guarantee. If you don't see results, contact us for a full refund. We believe in our product and want you to be completely satisfied.
              </Accordion>
              <Accordion title="Does It Work For My Skin Type?">
                Yes! Our products are hypoallergenic and tested on all skin types, including sensitive skin. They are free from harsh chemicals and gentle enough for daily use.
              </Accordion>
            </div>
